process COMPUTE_ELEMENT_STATS {
    ...
    output:
    path "coverage*All.txt", emit: coverage_stats, optional: true
    path "mmRate*All.txt", emit: mmrate_stats, optional: true
    path "processedExperiments*.csv", emit: processed_stats, optional: true
    path "*_collapse.bed", emit: collapsed_elements, optional: true
    ...
    for script in process_replicate_signal.py; do
    ...
    echo "=== Checking replicate availability for Steps 2 and 3 ==="
    ...
    if [[ ${dms_count} -eq 3 && ${stress_dms_count} -eq 3 ]]; then
        ...
        echo "=== Step 2: Creating final output files ==="
    ...
    # Extract data from each experiment
            awk 'BEGIN{OFS="\\t"} {print \$1,\$2,\$3,\$4,\$6,\$5}' \${dms_array[0]}_\${assay}_${reference_tag}_collapse.bed > \${dms_array[0]}.tmp
            awk '{print \$5}' \${dms_array[1]}_\${assay}_${reference_tag}_collapse.bed > \${dms_array[1]}.tmp
            awk '{print \$5}' \${dms_array[2]}_\${assay}_${reference_tag}_collapse.bed > \${dms_array[2]}.tmp
            awk '{print \$5}' \${stress_dms_array[0]}_\${assay}_${reference_tag}_collapse.bed > \${stress_dms_array[0]}.tmp
            awk '{print \$5}' \${stress_dms_array[1]}_\${assay}_${reference_tag}_collapse.bed > \${stress_dms_array[1]}.tmp
            awk '{print \$5}' \${stress_dms_array[2]}_\${assay}_${reference_tag}_collapse.bed > \${stress_dms_array[2]}.tmp
    ...
    # Combine all experiments
            pr -m -t -J \${dms_array[0]}.tmp \${dms_array[1]}.tmp \${dms_array[2]}.tmp \${stress_dms_array[0]}.tmp \${stress_dms_array[1]}.tmp \${stress_dms_array[2]}.tmp | \\
                ...
                \${assay}_${reference_tag}_All.tmp
        
    # Add header
            cat "\$HEADER_DIR/\${assay}Head6.txt" \${assay}_${reference_tag}_All.tmp > \${assay}_${reference_tag}_All.txt
    ...
    # Clean up temporary files
            ...
            rm -f \${assay}_${reference_tag}_All.tmp
        
	    final_lines=`wc -l < \${assay}_${reference_tag}_All.txt`
            echo "Final \$assay file: \$final_lines lines"
        done
    
        echo "=== Step 3: Processing replicate signals ==="
    
    # Final step: process replicate signals
        python ${projectDir}/bin/process_replicate_signal.py \\
            --covin coverage_${reference_tag}_All.txt \\
            --mmin mmRate_${reference_tag}_All.txt \\
        ...
        echo "Coverage stats: `wc -l < coverage_${reference_tag}_All.txt` lines"
        echo "mmRate stats: `wc -l < mmRate_${reference_tag}_All.txt` lines"
        ...
        echo "Skipping Steps 2 and 3 - only individual gene files will be available"
